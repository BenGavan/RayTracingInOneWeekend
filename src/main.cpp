...
double hit_sphere(const point3 &center, double radius, const ray &r) {
    ...
    double a = r.direction().length_squared(); // b^2  (b from P(t) = A + t*b)
    double h = dot(oc, r.direction());  // (A-C).b  (half of b in QE)
    ...
    if (discriminant < 0) {
        ...
        return (-h - sqrt(discriminant)) / a;  // smallest value of t (i.e. the closest intersection point)
    }
}
